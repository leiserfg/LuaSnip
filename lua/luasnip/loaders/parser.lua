...
local regex_fragment = map(many(any(pattern([[^\/]]), pattern("^[^/]"))), function(v) 
  return (table.concat(v, ''))
end)
local regex_option = map(many(any(pattern([[^\}]]), pattern("^[^}]"))), function(v) 
  return (table.concat(v, ''))
end)
...
variable = any(
  ...
  map(
    seq(dollar, open, var, regex, close),
...
function parse(snippet_text)
  ...
  if p == 1 + #snippet_text and s then return v end
  error(("%s is not a vaild snippet, error near %d %s"):format(#snippet_text, p, vim.inspect(v)))
end
print(vim.inspect( parse([[hello $1${2} ${1|one,two,three|} ${name:foo} $var $TM ${1: $TM_SELECTED_TEXT }  ${TM_FILENAME/(.*).+$\/test/$1/g} ${3:$4}]])))
-- 
