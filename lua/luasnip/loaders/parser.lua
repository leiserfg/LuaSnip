...
local int = map(pattern("^%d+"), function(v)
	return tonumber(v)
end)

local text = take_until

-- either no slashes or escaped slashes
local regex_fragment = map(
	many(any(pattern([[^\/]]), pattern("^[^/]"))),
	function(v)
		return (table.concat(v, ""))
	end
)
local regex_option = map(
	many(any(pattern([[^\}]]), pattern("^[^}]"))),
	function(v)
		return (table.concat(v, ""))
	end
...
local regex = map(
	seq(slash, regex_fragment, slash, regex_fragment, slash, regex_option),
	function(v)
		return { type = "regex", value = v[1], format = v[2], options = v[3] }
	end
)
...
tabstop = map(any(seq(dollar, int), seq(dollar, open, int, close)), function(v)
	return { type = "tabstop", id = v[1] }
end)
...
placeholder = map(
	...
	function(v)
		return { type = "placeholder", id = v[1], value = v[2] }
	end
)
...
choice = map(
	...
	function(v)
		return { type = "choice", id = v[1], value = v[2] }
	end
)

variable = any(
	map(seq(dollar, var), function(v)
		return { type = "variable", name = v[1] }
	end),
	map(
		seq(dollar, open, var, colon, many(any(anything, text("}"))), close),
		function(v)
			return { type = "variable", name = v[1], default = v[2] }
		end
	),
	map(seq(dollar, open, var, regex, close), function(v)
		return { type = "variable", name = v[1], regex = v[2] }
	end)
)

-- toplevel text matches until $
local snippet = many(any(anything, text("%$")))

function parse(snippet_text)
	local s, v, p = snippet(snippet_text, 1)
	if p == 1 + #snippet_text and s then
		return v
	end
	error(
		("%s is not a vaild snippet, error near %d %s"):format(
			#snippet_text,
			p,
			vim.inspect(v)
		)
...
print(
	vim.inspect(
		parse(
			[[hello $1${2} ${1|one,two,three|} ${name:foo} $var $TM ${1: $TM_SELECTED_TEXT }  ${TM_FILENAME/(.*).+$\/test/$1/g} ${3:$4}]]
		)
	)
)
--
-- local s, v, p = parse("local ${1:var} = ${1:value}", 1)
