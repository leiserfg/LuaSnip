...
* `opts` is a table containing (in every case `EnvVal` is the same as `string|list[string]`:
    ...
    * `init`: `fn(meta: pair[int])->map[string, EnvVal]`  Returns
        ...
        The `meta` argument contains the 0 based position of the cursor when expanding in `meta[1]` and `meta[2]`, 
        and more snippet data as keys (till now `trigger` and `captures`).
